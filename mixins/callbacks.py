...
class CallbacksMixin:
    ...
    def __init__(self):
        ...
        self.__callbacks: Dict[
            str, List[Dict[str, Union[Callable[..., Any], Optional[Dict[str, Any]]]]]
        ] = {}
    ...
    def unregister_callback(self, event: str, callback: Callable[..., Any]) -> None:
        self.__callbacks[event] = [
            cb for cb in self.__callbacks[event] if cb["callback"] != callback
        ]

    def trigger_all_callbacks(self, category: str, *args: Any, **kwargs: Any) -> None:
        self.trigger_callback(
            category=category, index=self.ALL_CALLBACKS, *args, **kwargs
        )

    def trigger_callback(
        self, category: str, index: int = ALL_CALLBACKS, *args: Any, **kwargs: Any
    ) -> None:
        if len(self.__callbacks[category]) == 0:
            return

        def _trigger(
            item: Any | Callable[..., Any],
            item_kwargs: Any | Optional[Dict[str, Any]],
            *args: Any,
            **kwargs: Any,
        ) -> None:
            LogManager.get_instance().engine.debug(
                f"Triggering callback: {item.__name__}"
            )
        ...
        if index == self.ALL_CALLBACKS:
            for cb_dict in self.__callbacks[category]:
                _trigger(
                    cb_dict["callback"],
                    item_kwargs=cb_dict.get("kwargs"),
                    *args,
                    **kwargs,
                )
        else:
            ...
            _trigger(
                cb_dict["callback"], item_kwargs=cb_dict.get("kwargs"), *args, **kwargs
            )

    def register_callback(
        self, event: str, callback: Callable[..., Any], **kwargs: Any
    ) -> None:
        LogManager.get_instance().engine.debug(
