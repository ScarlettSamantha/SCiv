...
class ResearchButton(TooltippedButton):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.is_researching:
            self._cost_text = f"{str(PlayerManager.session_player().tech.current_science)}/{str(self.cost)}"
        else:
            self._cost_text = f"{str(self.cost)}"
    ...
    def _update_fade_opacity(self, *args: Any) -> None:
        self._fade_color.a = self.fade_alpha

    def _update_fade_rect(self, *args: Any) -> None:
        self._fade_rect.pos = self.pos
        self._fade_rect.size = self.size

    def _on_primary_text(self, _instance: Any, new: str) -> None:  # type: ignore
        if hasattr(self, "_name_lbl"):
            self._name_lbl.text = new

    def _refresh_second_line(self, *args: Any) -> None:
        ...
        def _update_width(_lbl: Label, size: Tuple[float, float]) -> None:  # type: ignore
            lbl.width = int(size[0])
    ...
    def destroy(self) -> None:
        for widget in self._title_row.children:
            if hasattr(widget, "destroy"):
                widget.destroy()  # type: ignore

        for widget in self._second_line.children:
            if hasattr(widget, "destroy"):
                widget.destroy()  # type: ignore
...
class Research(FloatLayout, DirectObject):
    ...
    def build(self) -> None:
        ...
        self._float_layout = FloatLayout(
            size_hint=(None, 1),
            pos_hint={"top": 0, "y": 0},
        )
    ...
    def _place_tech_buttons(self) -> None:
        ...
        for level in sorted(tech_by_level.keys()):
            if level == 0:
                sorted_tech_by_level[level] = sorted(
                    tech_by_level[level],
                    key=lambda tech: tech.key,
                )
            else:
        ...
        self._rebuild_column_lanes()
        self._calculate_button_state()

    def _get_column_spans(self, col: int) -> List[Tuple[float, float]]:
        spans: List[Tuple[float, float]] = []

        for tech_cls in self._tech_by_level.get(col, []):
    ...
    def _rebuild_column_lanes(self) -> None:
        self._column_lanes = {}
        ...
        for col in self._tech_by_level.keys():
            spans = self._get_column_spans(col)

            lanes: List[float] = []

            for i in range(len(spans) - 1):
                lower_top = spans[i][1] + margin
                upper_bottom = spans[i + 1][0] - margin

                gap = upper_bottom - lower_top
                if gap < min_gap:
                    continue

                lane_count = min(max_lanes_per_gap, int(gap // min_gap))
                if lane_count <= 0:
                    continue

                step = gap / (lane_count + 1)

                for j in range(lane_count):
                    lanes.append(lower_top + step * (j + 1))

            self._column_lanes[col] = lanes
    ...
    def _clear_highlight(self, *args: Any) -> None:
    ...
    def on_research_button_click(self, btn: ResearchButton) -> None:
        ...
        MessengerGlobal.messenger.send(
            "game.gameplay.research.request_start_research_session_player",
            [btn.value],
        )
    ...
    def _draw_dependency_lines(self, *args: Any) -> None:
        ...
        column_lane_indices: Dict[int, int] = defaultdict(int)
        ...
        with self._float_layout.canvas.before:
            for tech_cls in self._tech_classes:
                ...
                for req_cls in tech_cls.requires:
                    ...
                    per_edge_lane_indices: Dict[int, int] = {}

                    start_col = min(src_level, tgt_level)
                    end_col = max(src_level, tgt_level)

                    for col in range(start_col, end_col + 1):
                        lane_idx = column_lane_indices[col]
                        column_lane_indices[col] = lane_idx + 1
                        per_edge_lane_indices[col] = lane_idx

                    color_val = Colors.sequence()
                    color_instr = Color(*color_val)

                    points = self._build_stepwise_path(
                        src_level,
                        tgt_level,
                        sx,
                        sy,
    ...
    def _avoid_column_collision(self, col: int, y: float) -> float:
        spans = self._get_column_spans(col)
        if not spans:
            return y

        margin = dp(6)
        height = self._float_layout.height  # type: ignore

        for y0, y1 in spans:
            if y0 - margin <= y <= y1 + margin:
                above = max(margin, y0 - margin)
                below = min(height - margin, y1 + margin)

                if abs(y - above) <= abs(y - below):
                    y = above
                else:
                    y = below
    ...
    def _get_route_y_for_column(
        ...
        target_weight = 0.35
        base_y = current_y * (1.0 - target_weight) + target_y * target_weight

        bucket_count = 7
        bucket_index = (lane_index % bucket_count) - (bucket_count // 2)
        jitter = dp(5) * bucket_index

        candidate = base_y + jitter

        max_step = dp(70)
        delta = candidate - current_y

        if delta > max_step:
            candidate = current_y + max_step
        elif delta < -max_step:
            candidate = current_y - max_step

        return self._avoid_column_collision(col, candidate)

    def _adjust_arrival_y_for_target(
        self,
        ...
        bottom = to_btn.y + margin  # type: ignore
        top = to_btn.top - margin  # type: ignore

        if bottom > top:
            center = to_btn.center_y  # type: ignore
            return center
        ...
        bucket_count = 5
        bucket_index = (lane_index % bucket_count) - (bucket_count // 2)
        jitter = dp(4) * bucket_index

        adjusted = base + jitter
    ...
    def _build_stepwise_path(
        ...
        to_btn: ResearchButton,
        arrival_y: float,
        column_lane_indices: Dict[int, int],
    ) -> List[float]:
        target_lane_index = column_lane_indices.get(tgt_level, 0)
        arrival_y = self._adjust_arrival_y_for_target(to_btn, arrival_y, target_lane_index)
        ...
        if abs(tgt_level - src_level) == 1:
            if going_right:
                ...
                base_mid = sx + dp(40)
                mid_x = min(base_mid, safe_end_x - self._adjacent_mid_margin)
                target_x = contact_x
            else:
                safe_end_x = contact_x + (self._arrow_length + head_clearance)
                base_mid = sx - dp(40)
                mid_x = max(base_mid, safe_end_x + self._adjacent_mid_margin)
                target_x = contact_x
        ...
        while col != tgt_level:
            ...
            lane_idx = column_lane_indices.get(next_col, 0)
            route_y = self._get_route_y_for_column(next_col, current_y, arrival_y, lane_idx)
