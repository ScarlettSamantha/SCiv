...
class Rationalism(BaseCoreSubtree):
    ...
    def register_civics(cls) -> List[Type[Civic]]:
        ...
        scientific_inquiry.set_requirements([CivicCondition(reason_and_logic)])
        ...
        secular_governance.set_requirements([CivicCondition(reason_and_logic)])
        secular_governance.tier = 1
        cls.add_civic(secular_governance)

        education_reform = EducationReform
        education_reform.set_requirements([CivicCondition(scientific_inquiry)])
        ...
        evidence_based_policy.set_requirements([CivicCondition(education_reform)])
        evidence_based_policy.tier = 3
        cls.add_civic(evidence_based_policy)

        philosophical_discourse = PhilosophicalDiscourse
        philosophical_discourse.set_requirements([CivicCondition(evidence_based_policy)])
