...
class Lake(BaseTerrain):
    ...
    def register_bits(self) -> None:
        ...
        lake_plants_d = Bit(model="water_plants_4.glb", scale=1.0, offset=(0, 0, 0))
        lake_plants_e = Bit(model="water_plants_5.glb", scale=1.0, offset=(0, 0, 0))

        self.bits.empty_probability = 0.55

        self.bits.add_group("lake_plants").add_bit(lake_plants_a.rotate(randint(0, 6) * (360 / 6)).copy())
        ...
        plant_3.add_bit(lake_plants_b.copy())
        plant_3.add_bit(lake_plants_a.copy())

        plant_4: Bits = self.bits.add_group("lake_plants_4")
        plant_4.add_bit(lake_plants_c.copy())
        plant_4.add_bit(choice([lake_plants_a.copy(), lake_plants_b.copy()]))

        plant_5: Bits = self.bits.add_group("lake_plants_5")
        plant_5.add_bit(lake_plants_d.copy())
        plant_5.add_bit(lake_plants_a.copy())
        plant_5.add_bit(lake_plants_b.copy())

        plant_6: Bits = self.bits.add_group("lake_plants_6")
        plant_6.add_bit(lake_plants_e.copy())
        plant_6.add_bit(lake_plants_a.copy())
        plant_6.add_bit(lake_plants_b.copy())
